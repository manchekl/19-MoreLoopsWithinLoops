...
def run_test_largest_number():
    ...
    expected = 15
    answer = largest_number([(13, 15, 14),
                             (3, 0, 3, 8, 10),
                             [1, 1, 1, 1]])
    print('Expected and actual are:', expected, answer)
...
def largest_number(seq_seq):
    ...
    for s in range (len(seq_seq)):
        for num in range (len(seq_seq[s])):
            tree=seq_seq[s]
            if large==None:
                large=tree[num]
            if tree[num]>large:
...
def run_test_largest_negative_number():
    ...
    # DONE 2 (continued): Add your ADDITIONAL test(s) here:
    expected = -15
    answer = largest_negative_number([(13, -15, 14),
                             (-3, 0, 3, -8, 10),
                             [1, -1, 1, -1]])
    print('Expected and actual are:', expected, answer)
...
def first_is_elsewhere_too(seq_seq):
    ...
    for k in range(len(seq_seq[0])):
        for w in range (1,len(seq_seq)):
            for s in range (len(seq_seq[w])):
                if (seq_seq[0][k])==(seq_seq[w][s]):
                    truth =True
                    break
